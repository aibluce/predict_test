...
见第四节表格（每条都给理由）。最影响输出的三条是：挂单方限价成交、同有效价按挂单先后、合成价更优优先；
这三条都写在 `MatchEngine` 的注释里，改动点集中，评分方若按我的说明重生成参考输出，只需要看那一处。
第四层的两条（`FEE_RATE 0` 允许、rate 输出用 `toPlainString`）只影响多一行 `FEE_RATE_SET`。

### 2. 我放弃了哪个方案，为什么

**放弃一：把两个 outcome 合成维护成一张「有效盘口」。** 一开始想让 `bestBid / bestAsk` 和撮合都直接在
一张 `TreeMap` 上查，价格 O(log n) 直接读、撮合零归并成本。问题是每来一张挂单，另一 outcome 的合成价位
都要跟着改（`BUY YES @0.60` 挂上就等于往 NO 的卖盘里插一个 0.40 的虚拟价位），撤单/成交时还要精准撤销，
一旦漏一处，盘口就出现幽灵流动性；而且第三层做快照时要深拷贝两套互相推导的结构，很容易和 replay 结果不一致。

改成「只存原始盘口 + 查询/撮合时算合成价」：代价是 `bestBid/bestAsk` 各多读一次对面盘口的边界值、
撮合时要归并两个来源；换来的是**状态无冗余**（快照只需要存一张盘口，两份快照深比较就能证明重放一致），
以及「合成价只在算的时候存在，不可能变成脏数据」。

**放弃二：先吃 NORMAL、吃不动了再吃 MINT/MERGE。** 实现最简单，但违反价格优先——用 0.70 买单时
明明能以 0.01 铸造，却先去成交 0.69。

**放弃三：快照只存事件列表（restore 就等于从头 replay）。** 这样只要几行代码，但 `restore` 是 O(事件数)，
而且没有真正验证「状态能独立于日志被保存」。现在的快照是真状态深拷贝，代价是 `Snapshot` 得显式列出所有
状态字段——这也是它的用处：漏了哪个字段，两两深比较就会立刻暴露。

### 3. 我自己的随机测试找出了我几个 bug

**到目前为止 0 个**（每一轮随机测试都是第一次跑就通过）。写的时候靠自查抓到过一个真会算错的 bug，
记在这里免得复盘时说不清：`MatchEngine` 里两个来源的遍历方向，一开始我按「taker 是 BUY 就升序遍历」
统一写了，结果买 MINT 那一支会去扫**最低**的买单（应该扫最高买价，因为 `1 − r` 才是 taker 的价），
一挂上 `BUY NO @0.10` 就会以 0.90 跟买方成交，直接违反「不比限价差」。现在方向由
`ascending(side, type)` 显式决定，并有「合成价更优时_优先吃合成价」这个定向用例守着。

第四层的 `released ≥ 0` 我一开始也不确定会不会有反例，于是先把「ceil 与 floor 互相抵消」的证明写出来
（见 3.5），再让 `Ledger.settleBuy` 在 `release < 0` 时抛异常，用两个刁钻费率各跑 10 万次随机操作去证伪——
一次都没触发；反倒是同一个公式换成四舍五入时，用手算就找到了 `−0.0001` 的反例（已写成用例）。

另外有一个不是随机测试找出来、而是**实跑文档里的命令**才暴露的 bug：`build.gradle` 的评分脚本任务原本写的是
`args = [project.findProperty("script") ?: "scripts/sample_nofee.txt"]`，在 Gradle 9 下这个 `findProperty("script")`
会拿到 `false`（属性名和脚本自身的 `script` 撞了），于是 `-Pscript=scripts/sample_fee.txt`
被静默忽略、永远跑默认脚本——输出看着"正常"，但跑的不是我指定的那个脚本。
改成 `providers.gradleProperty("script").getOrElse(...)` 后正确（顺带也对配置缓存友好）。
教训是：随机测试再密也测不到构建脚本的参数传递，交付前必须把 README 里写的每条命令真跑一遍。

### 4. 哪些代码是 AI 生成的，我改了什么，为什么改

题二的全部代码（`exchange` 包与它的测试）由 AI 在本会话里生成；人做的决定是：分两批交付的范围
（先第一、二层 + 适配器，再第三、四层）、第四节表格里的每一条默认选择、快照要和哪些字段一起深比较，
以及最终的复核与实跑。动手改过的部分请继续往这一节追加（改了什么、为什么改）。
